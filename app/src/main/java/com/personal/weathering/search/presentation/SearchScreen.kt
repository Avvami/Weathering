...
import com.personal.weathering.core.presentation.components.ThinLinearProgressIndicator
...
fun SearchScreen(
    ...
    Scaffold(
        ...
        SearchBar(
            ...
            onQueryChange = { searchViewModel.searchUiEvent(
                SearchUiEvent.OnSearchQueryChange(
                    it,
                    preferencesState.value.searchLanguageCode
                )
            ) },
            onSearch = { searchViewModel.searchUiEvent(
                SearchUiEvent.OnSearchQueryChange(
                    it,
                    preferencesState.value.searchLanguageCode
                )
            ...
            trailingIcon = {
                Row {
                    AnimatedVisibility(
                        ...
                        IconButton(
                            onClick = { searchViewModel.searchUiEvent(
                                SearchUiEvent.OnSearchQueryChange(
                                    "",
                                    preferencesState.value.searchLanguageCode
                                )
                    ...
                    IconButton(
                        onClick = { searchViewModel.searchUiEvent(
                            SearchUiEvent.SetLanguageDropdownExpanded(
                                true
                            )
                        ) }
                    ) {
                            ...
                            dropDownItems = SearchLanguage.languages.map {
                                DropdownItem(
                                    ...
                                    onItemClick = {
                                        ...
                                        searchViewModel.searchUiEvent(
                                            SearchUiEvent.SetSearchLanguage(
                                                it.code
                                            )
                                        )
                                    }
